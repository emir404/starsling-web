...
function appearT(reduce: boolean, delay: number, duration = 0.5): Transition {
  return reduce ? { duration: 0.3 } : { duration, ease: EASE, delay };
}

const HIDE_T: Transition = { duration: 0.3, ease: EASE };

/**
 * Shown/hidden wrapper for DOM scene elements: appear with the element's
 * choreography delay (blur + position + opacity), exit quickly and uniformly
 * when the cycle wraps. `initial={false}` so the first paint is the beat-0
 * state and only the band-level entrance animates on load.
 */
function Reveal({
  shown,
  delay,
